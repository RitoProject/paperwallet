ninja.wallets.detailwallet = {
		...
		if (ninja.privateKey.isBIP38Format(key)) {
			...
			ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(key, passphrase, function (btcKeyOrError) {
				...
				if (btcKeyOrError.message) {
					...
					ninja.wallets.detailwallet.populateKeyDetails(new Ritocoin.ECKey(btcKeyOrError));
				}
			});
		}
		else {
			if (Ritocoin.ECKey.isMiniFormat(key)) {
			...
			else if (Ritocoin.ECKey.isBase6Format(key)) {
	...
	populateKeyDetails: function (btcKey) {
		if (btcKey.priv != null) {
			...
			var bitcoinAddress = btcKey.getRitocoinAddress();
			var wif = btcKey.getRitocoinWalletImportFormat();
			...
			var bitcoinAddressComp = btcKey.getRitocoinAddress();
			var wifComp = btcKey.getRitocoinWalletImportFormat();
